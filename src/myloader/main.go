...
var (
	...
	flagUser, flagPasswd, flagHost, flagDir, flagDB string
...
func init() {
	...
	flag.StringVar(&flagDB, "db", "","Target database")
}

func usage() {
	fmt.Println("Usage: " + os.Args[0] + " -h [HOST] -P [PORT] -u [USER] -p [PASSWORD] -d [DIR] [-o] [-db]")
	flag.PrintDefaults()
}
